...
1. **Plan** — choose **City & country** or **Coordinates & radius** coverage. All countries are suggested as you type, and picking one suggests its major cities (any city name still works). Add one search term per line, or click the curated presets (**Core**, **Extended**, **Long tail**, **Civic**) — a preset click adds its terms, clicking again removes them. Presets are trimmed so overlapping categories ("doctors"/"clinics", "hair salons"/"beauty salons") are represented by a single term. Terms run serially in the displayed order, up to 50 per run.
...
6. **Run** — watch the live stage, job progress, a "~X min left" countdown of the whole run, and the countdown to the next search. Pip the compass doubles as the cooldown timer (its ring depletes second by second) and offers commentary along the way. Stop at any time; collected data is kept.
